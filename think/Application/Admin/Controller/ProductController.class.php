...
class ProductController extends PublicController {
	//定义图片路径：
	public $path= './Uploads/Product/';//上传文件的路径文件夹
   ...
   public function index() {
   	...
   	$list = $User->field("product.*,category.name")->join('LEFT JOIN __CATEGORY__ ON __PRODUCT__.category = __CATEGORY__.id' )->order('time DESC')->limit($Page->firstRow.','.$Page->listRows)->select();
   	$p=I("get.p",1);
   ...
   public function insert(){
   	...
   	if (IS_POST){
   		//print_r($_POST);die;
   		$thickrepertory=$_POST["thickrepertory"];
   		$numthick=array_sum($thickrepertory);
   		//print_r($numthick);die;
   		
   		$sizerepertory=$_POST["sizerepertory"];
   		$numsize=array_sum($sizerepertory);
   		
   		$colorrepertory=$_POST["colorrepertory"];
   		$numcolor=array_sum($colorrepertory);
   		
   		if ($numsize>$numthick){
   			
   			$this->error("尺码输入库存总数超过厚度之和！");
   		}
   		
   		if ($numcolor>$numthick){
   		
   			$this->error("颜色输入库存总数超过厚度之和！");
   		}
   		
   		$thickness=$_POST["thickness"];
   		$_POST["thickness"]=json_encode($thickness);
   		$size=$_POST["size"];
   		$_POST["size"]=json_encode($size);
   		$introduce=$_POST["introduce"];
   		$_POST["introduce"]=json_encode($introduce);
   		
   		$thickrepertory=$_POST["thickrepertory"];
   		$_POST["thickrepertory"]=json_encode($thickrepertory);
   		$sizerepertory=$_POST["sizerepertory"];
   		$_POST["sizerepertory"]=json_encode($sizerepertory);
   		$colorrepertory=$_POST["colorrepertory"];
   		$_POST["colorrepertory"]=json_encode($colorrepertory);
   		 //print_r($_POST);die;
   		//print_r($_FILES);die; 
   		...
   		//调用新增：
   		$result=M("product")->data($_POST)->add();
   		if ($result>0){
 ...
 Public function update(){
  if(IS_GET){
   		...
   		$User = M("product"); // 实例化User对象
	   	 ...
	   	 $arr["thickness"]=json_decode($thickness);
	   	 $size=$arr["size"];
	   	 $arr["size"]=json_decode($size);
	   	 $images=json_decode($arr["images"]);
	   	 $arr["images"]=$images;
	   	 $introduce=$arr["introduce"];
	   	 $arr["introduce"]=json_decode($introduce);
	   	 
	   	 $thickrepertory=$arr["thickrepertory"];
	   	 $arr["thickrepertory"]=json_decode($thickrepertory);
	   	 $sizerepertory=$arr["sizerepertory"];
	   	 $arr["sizerepertory"]=json_decode($sizerepertory);
	   	 $colorrepertory=$arr["colorrepertory"];
	   	 $arr["colorrepertory"]=json_decode($colorrepertory);
